...
def normalize_numeric_token(s):
    """Return canonical integer-like string if numeric; keep leading zeros; reject alpha/underscores/fractional decimals."""
    if s is None:
        return None
    s = str(s).strip()
    if not s:
    ...
    if _INT_OR_WHOLE_FLOAT.fullmatch(s):
        return s.split('.', 1)[0]  # strip ".0"
    return None

def first_valid_numeric_from_list(s):
    """Return first valid numeric token from a comma/semicolon list; else None."""
    if s is None:
        return None
...
def extract_size_and_unit(name):
    """
    Extract (clean_name, size, size_uom) from end of product name.
    Allowed units: g, lb, gr, kg, pk, pack, each, oz, mg, ml, l
    If ends with 'each', try second-last token like '170g'.
    Defaults to (name, 1, 'each') if nothing matches.
    """
    ...
    # If ends with 'each', try second-last token
...
def get_lookup_code(row, product_code_col: str, sku_col: str) -> str:
    """
    Compare 'code' vs 'sku':
      - Accept only numeric tokens (e.g., '0012', '182', '182.0' -> '182').
      - If list, take first valid numeric token.
      - If both numeric, keep the larger integer; return normalized string (preserve zeros).
      - Else blank.
    """
...
def retail_round(x: float) -> float:
    """
    Round a price UP to the nearest .29, .49, .79, or .99 after applying margin/markup.
    """
    if pd.isna(x):
        return x
    base = math.floor(x)
...
def process_mapped_items(input_file: str) -> pd.DataFrame:
    ...
    # Read as strings for code/sku to avoid floatification
    ...
    # Apply 13% markup (your current request) then retail rounding
    # If you wanted a TRUE 13% margin instead, replace '* 1.13' with '/ 0.87'
    ...
    # --- Pack rule: 'pk'/'pack' -> move size to unit_count; set size=1
    ...
    # Drop blanks (non-numeric IDs)
    out = out[out["lookup_code"].astype(str).str.strip() != ""].copy()

    # Deduplicate by lookup_code (keep highest price)
    work = out.copy()
...
def save_outputs(df_out: pd.DataFrame, xlsx_path: str, csv_date_override: str | None = None):
    # Optional XLSX with text formatting for lookup_code
    ...
    # Ensure leading zeros survive by wrapping as ="value"
    df_csv = df_out.copy()
